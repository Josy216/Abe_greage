...
async function getAllOrders(page, limit, searchTerm) {
...
async function getOrdersCount(searchTerm) {
    let query = `
...
async function createOrder(order) {
    try {
        ...
        const orderHash = (0, crypto_1.randomBytes)(16).toString('hex');
        const orderQuery = 'INSERT INTO orders (customer_id, employee_id, vehicle_id, order_date, active_order, order_hash) VALUES (?, ?, ?, NOW(), ?, ?)';
        const orderResult = await db_config_1.default.query(orderQuery, [
            order.customer_id,
            order.employee_id,
            order.vehicle_id,
            1, // active_order
        ...
        const orderInfoQuery = 'INSERT INTO order_info (order_id, order_total_price, estimated_completion_date, completion_date, additional_request, additional_requests_completed) VALUES (?, ?, ?, ?, ?, ?)';
        await db_config_1.default.query(orderInfoQuery, [
            orderId,
        ...
        await db_config_1.default.query(orderStatusQuery, [orderId, 3]); // 1 for 'Pending' status
        ...
        if (order.order_services && order.order_services.length > 0) {
            ...
            for (const service of order.order_services) {
                await db_config_1.default.query(servicesQuery, [orderId, service.service_id, 0]);
            }
        }
...
async function updateOrder(orderData) {
    try {
        const { order_description, estimated_completion_date, completion_date, order_services, order_status, } = orderData;
        const orderId = orderData.order_id;
        // Check if order exists
        const rows = await db_config_1.default.query('SELECT order_id FROM orders WHERE order_id = ?', [orderId]);
        if (!rows || rows.length === 0) {
            throw new Error(`Order with id ${orderId} does not exist`);
        }
        // 1. Update order status if provided
        if (order_status !== undefined) {
            await db_config_1.default.query('UPDATE order_status SET order_status = ? WHERE order_id = ?', [order_status, orderId]);
        }
        // 2. Delete existing order services
        await db_config_1.default.query('DELETE FROM order_services WHERE order_id = ?', [
            orderId,
        ]);
        await db_config_1.default.query('UPDATE order_status SET order_status = ? WHERE order_id = ?', [order_status, orderId]);
        // 3. Update order info
        await db_config_1.default.query(`UPDATE order_info 
       SET additional_request = ?, 
           estimated_completion_date = ?, 
           completion_date = ?
       WHERE order_id = ?`, [order_description, estimated_completion_date, completion_date, orderId]);
        // 4. Insert new order services
        if (order_services && Array.isArray(order_services)) {
            for (const service of order_services) {
                await db_config_1.default.query('INSERT INTO order_services (order_id, service_id, service_completed) VALUES (?, ?, ?)', [
                    orderId,
                    service.service_id,
...
`;
    try {
        const [rows] = await db_config_1.default.query(query, [hash]);
        let order = null;
...
async function getOrdersByUserId(userId) {
    try {
        ...
        const [customerCheck] = await db_config_1.default.query('SELECT customer_id FROM customer_identifier WHERE customer_id = ?', [userId]);
      ...
      ORDER BY o.order_date DESC;
    `;
        const rows = await db_config_1.default.query(query, [userId]);
        const orders = rows.map((order) => {
            if (order.services) {
                order.services = JSON.parse(`[${order.services}]`);
            }
...
async function deleteOrder(orderId) {
    try {
        const [rows] = await db_config_1.default.query('SELECT * FROM orders WHERE order_id = ?', [orderId]);
        if (rows.length === 0) {
            throw new Error(`Order with id ${orderId} does not exist`);
        }
        // Step 1: Delete from order_status
        await db_config_1.default.query('DELETE FROM order_status WHERE order_id = ?', [orderId]);
        // Step 2: Delete from order_services
        await db_config_1.default.query('DELETE FROM order_services WHERE order_id = ?', [orderId]);
        // Step 3: Delete from order_info
        await db_config_1.default.query('DELETE FROM order_info WHERE order_id = ?', [orderId]);
        // Step 4: Finally, delete from orders
        await db_config_1.default.query('DELETE FROM orders WHERE order_id = ?', [orderId]);
        return true;
    }
    catch (error) {
        throw error;
