...
async function checkIfCustomerExists(email: string) {
  ...
  const rows = await conn.query<any[]>(query, [email]);
...
async function getCustomersCount(searchTerm: string) {
  let query = `
...
async function getCustomerById(customerId: number) {
  const query = `
    ...
    WHERE 
      ci.customer_id = ?`;
  const rows = await conn.query<any[]>(query, [customerId]);
...
async function updateCustomer(customerId: number, customer: any) {
  try {
    ...
    const query2 = `
      ...
      WHERE 
        customer_id = ?`;
    await conn.query(query2, [customer.customer_phone_number, customerId]);
...
async function deleteCustomer(id: number) {
  try {
    ...
    const query =
      'SELECT COUNT(*) as count FROM customer_identifier WHERE customer_id = ?';

    const countResult = await conn.query(query, [id]);
    ...
    if (Array.isArray(orders) && orders.length > 0) {
      ...
      if (orderIds.length > 0) {
        await conn.query('DELETE FROM order_services WHERE order_id IN (?)', [
          orderIds,
        ]);

        // Delete from order_status (depends on order_id)
        await conn.query('DELETE FROM order_status WHERE order_id IN (?)', [
          orderIds,
        ]);

        // Delete from order_info (depends on order_id)
        await conn.query('DELETE FROM order_info WHERE order_id IN (?)', [
          orderIds,
        ]);
      }
